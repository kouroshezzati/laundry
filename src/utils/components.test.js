...
describe('Utils', () => {
  it('it should add with two digits precise', () => {
    ...
    expect(calc(ADD, 0, '1.00')).toBe('1.00');
    expect(calc(ADD, '0', '1.00')).toBe('1.00');
    expect(calc(ADD, '1.00', '0')).toBe('1.00');
    expect(calc(ADD, '1.00', 0)).toBe('1.00');
  });
  ...
  it('should multiple price', () => {
    ...
    expect(multipleCurrency(1, 2)).toBe('2.00');
    expect(multipleCurrency(0, 2)).toBe('0.00');
    expect(multipleCurrency(1, 0)).toBe('0.00');
  });

  it('should be able to work together', () => {
    expect.assertions(1);
    expect(calc(ADD, '159.50', multipleCurrency(7, '9.50'))).toBe('226.00');
  });
});
